...
func TestDBSimple(t *testing.T) {
	...
	total := func(name internal.MetricName, opts ...internal.MetricOption) int64 {
	...
	for _, tc := range []struct {
		name internal.MetricName
		val  int64
	}{
		{internal.TotalFileCount, 4},
		{internal.TotalPrefixCount, 2},
		{internal.TotalDiskUsage, 80},
	} {
		if got, want := total(tc.name, internal.Global()), tc.val; got != want {
	...
	if got, want := total(internal.TotalDiskUsage, internal.Global()), int64(80); got != want {
		t.Errorf("got %v, want %v", got, want)
	}
	users, err = db.UserIDs(ctx)
	assertUsers(2)
...
func TestDBLocking(t *testing.T) {
	...
	_, err = localdb.Open(ctx, dbDir,
		[]internal.DatabaseOption{internal.ReadOnly()},
		localdb.LockStatusDelay(time.Millisecond*100),
		localdb.TryLock(),
	)
	if err == nil || !strings.Contains(err.Error(), "failed to lock") {
...
func fill(ctx context.Context, db internal.Database, when time.Time, n int) error {
	for i := 0; i < n; i++ {
...
func TestScanning(t *testing.T) {
	...
	keys := []string{}
	limit := 10
	sc := db.NewScanner("", limit, internal.ScanLimit(6))
	for sc.Scan(ctx) {
		k, v := sc.PrefixInfo()
		keys = append(keys, k)
	...
	sc = db.NewScanner("", 3333, internal.ScanLimit(6))
	keys = []string{}
	for sc.Scan(ctx) {
		k, v := sc.PrefixInfo()
		keys = append(keys, k)
