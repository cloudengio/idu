...
type Scanner struct {
	...
	ifcOpts internal.ScannerOptions
	...
	currentInfo     internal.PrefixInfo
	next            int
	availablePrefix []string
	availableInfo   []internal.PrefixInfo
	err             error
}
...
func NewScanner(db *Database, prefix string, limit int, ifcOpts []internal.ScannerOption, opts ...ScanOption) *Scanner {
	sc := &Scanner{
		pdb:    db.prefixdb,
		prefix: []byte(prefix),
		nItems: limit,
...
func (sc *Scanner) scanByPrefix(limit int) ([]string, []internal.PrefixInfo, error) {
...
func (sc *Scanner) scanByRange(limit int) ([]string, []internal.PrefixInfo, error) {
...
func (sc *Scanner) processKeys(keys [][]byte) ([]string, []internal.PrefixInfo, error) {
	if len(keys) == 0 {
		return nil, nil, nil
	}
	sc.offset += len(keys)
...
func getItems(db *pudge.Db, keys [][]byte) ([]string, []internal.PrefixInfo, error) {
	prefixes := make([]string, len(keys))
	info := make([]internal.PrefixInfo, len(keys))
	for i, key := range keys {
		prefixes[i] = string(key)
...
// Scan implements internal.DatabaseScanner.
...
var empty = &internal.PrefixInfo{}

// PrefixInfo implements internal.DatabaseScanner.
func (sc *Scanner) PrefixInfo() (string, *internal.PrefixInfo) {
	if sc.ifcOpts.KeysOnly {
...
// Err rimplements internal.DatabaseScanner.
