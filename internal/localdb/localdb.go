...
func Open(ctx context.Context, dir string, ifcOpts []internal.DatabaseOption, opts ...DatabaseOption) (internal.Database, error) {
	db := newDB(dir)
	var dbOpts internal.DatabaseOptions
...
func (db *Database) Set(_ context.Context, prefix string, info *internal.PrefixInfo) error {
	if db.opts.readOnly {
		return ErrReadonly
	}
...
func (db *Database) Get(_ context.Context, prefix string, info *internal.PrefixInfo) (bool, error) {
...
func (db *Database) NewScanner(prefix string, limit int, opts ...internal.ScannerOption) internal.DatabaseScanner {
...
func getMetricNames() []internal.MetricName {
	metrics := []internal.MetricName{
		internal.TotalFileCount,
		internal.TotalPrefixCount,
		internal.TotalDiskUsage,
		internal.TotalErrorCount,
	}
...
func (db *Database) statsForDb(pdb *pudge.Db, filename, name, desc string) (internal.DatabaseStats, error) {
	count, err := pdb.Count()
	if err != nil {
		return internal.DatabaseStats{}, fmt.Errorf("failed to get # entries for %v: %v", name, err)
	}
	size, err := pdb.FileSize()
	if err != nil {
		return internal.DatabaseStats{}, fmt.Errorf("failed to size %v for %v: %v", filename, name, err)
	}
	return internal.DatabaseStats{
		Name:        name,
		Description: desc,
...
func (db *Database) Stats() ([]internal.DatabaseStats, error) {
	stats := []internal.DatabaseStats{}
	errs := errors.M{}
...
func (db *Database) Metrics() []internal.MetricName {
	return getMetricNames()
}

func metricOptions(opts []internal.MetricOption) internal.MetricOptions {
	var o internal.MetricOptions
	for _, fn := range opts {
		fn(&o)
	}
	return o
...
func (db *Database) statsCollectionForOption(o internal.MetricOptions) (*statsCollection, error) {
	if o.Global {
...
func (db *Database) Total(_ context.Context, name internal.MetricName, opts ...internal.MetricOption) (int64, error) {
...
func (sc *statsCollection) total(name internal.MetricName) (int64, error) {
	switch name {
	case internal.TotalFileCount:
		return sc.NumFiles.Sum(), nil
	case internal.TotalPrefixCount:
		return sc.NumChildren.Sum(), nil
	case internal.TotalDiskUsage:
		return sc.DiskUsage.Sum(), nil
	case internal.TotalErrorCount:
		return sc.NumErrors, nil
	}
	return -1, fmt.Errorf("unsupported metric: %v", name)
}

func (db *Database) TopN(_ context.Context, name internal.MetricName, n int, opts ...internal.MetricOption) ([]internal.Metric, error) {
	o := metricOptions(opts)
	sc, err := db.statsCollectionForOption(o)
	if err != nil {
		return nil, err
...
func (sc *statsCollection) topN(name internal.MetricName, n int) ([]internal.Metric, error) {
	switch name {
	case internal.TotalFileCount:
